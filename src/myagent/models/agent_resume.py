...
class EntriesSection(Section):
    ...
    def validate(self) -> bool:
        ...
        if self.id == "experience":
            valid = valid and "Experience" in self.title and "Projects" not in self.title
        elif self.id == "projects":
            valid = valid and ("Project" in self.title or "Projects" in self.title) and "Experience" not in self.title
...
class Resume:
    ...
    def validate(self) -> bool:
        ...
        if not self.metadata or not self.metadata.first_name or not self.metadata.last_name:
            return False
...
def dict_to_resume(data: dict) -> Resume:
    ...
    # 处理额外的metadata字段
    known_fields = {'first_name', 'last_name', 'position', 'address', 'mobile', 'email', 'github', 'linkedin'}
...
if __name__ == "__main__":
    ...
    skills = SkillsSection(
        ...
        groups=[
            ...
            SkillGroup(
                category="Frameworks",
                items=["Django", "React", "FastAPI", "Next.js"]
            ),
            SkillGroup(
